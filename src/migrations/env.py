...
from infrastructure.database.models import *  # type: ignore # noqa
...
def process_revision_directives(context, revision, directives):  # type: ignore
    if not directives[0].upgrade_ops.is_empty():  # type: ignore
        print("✅ Обнаружены изменения, миграция создана")
    else:
        directives[:] = []  # Удаляем пустую миграцию
        print("❌ Нет изменений, миграция не создана")
